...
_STATUS_TOOLTIP = {
    "ready": _("已加载完成，可正常查询"),
    "loading": _("正在后台加载 key 索引，稍候即可查询"),
    "disabled": _("已取消启用，不参与查询（勾选可重新启用）"),
    "missing": _("原始 .mdx 文件找不到，需「修复路径」"),
}
...
class DictManagerDialog(QDialog):
    ...
    def __init__(self, service: DictionaryService, config=None,
        ...
        self._btn_delete = QPushButton(_("移除词库"))
    ...
    def refresh(self):
        ...
        for d in self._service.all_dicts():
            ...
            if status_key == "missing":
                ...
                reason = err or _("原始 .mdx 文件找不到（可能被改名 / 移动 / 删除）")
        ...
        # 缺失告警条
        if missing_rows:
            self._warn_label.setText(_(
                "⚠ 有 {} 部词库文件缺失（通常是文件夹被改名 / 移动后路径失效）：{}。"
                "\n选中后点「修复路径…」重新指定 .mdx 文件即可恢复。").format(
                    len(missing_rows), "、".join(missing_rows)))
    ...
    def _repair_selected(self):
        ...
        if dict_id is None:
            QMessageBox.information(self, _("修复路径"), _("请先选中一行词库。"))
            return
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, _("为「{}」选择 .mdx 文件").format(d.name),
            start_dir, "MDX 词库 (*.mdx);;所有文件 (*.*)")
        if not path:
            return
        try:
            self._service.repair_path(dict_id, path)
        except Exception as e:  # noqa: BLE001
            QMessageBox.critical(self, _("修复失败"), str(e))
            return
        self.refresh()
    ...
    def _add_folder(self):
        ...
        try:
            ...
            QMessageBox.critical(self, _("添加失败"), str(e))
            return
    ...
    def _report_add_result(self, registered: int, skipped: int, failed: list):
        ...
        if failed:
            ...
            skip_text = _("（跳过已添加 {} 部）").format(skipped) if skipped else ""
            QMessageBox.information(
                self, _("添加完成"),
                _("已添加 {} 部词库并开始后台加载。{}\n"
                  "加载完成后即可查询（列表「状态」列显示进度）。").format(
                    registered, skip_text))
