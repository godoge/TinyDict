...
from ..i18n import _, available_languages
...
class SettingsDialog(QDialog):
    def __init__(self, config: Config, parent=None):
        ...
        self._btn_reset_toggle = QPushButton(_("默认"))
        self._btn_reset_toggle.setToolTip(
            _("恢复为默认快捷键：") + str(Config.DEFAULTS["hotkey_show_hide"]))
        ...
        form.addRow(QLabel(_("显示 / 隐藏主窗口：")), toggle_row)

        # ---- 屏幕划词取词
        self._kse_capture = QKeySequenceEdit(
            QKeySequence(self._keyboard_to_sequence(
                str(self._config["hotkey_capture"]))))
        self._btn_reset_capture = QPushButton(_("默认"))
        self._btn_reset_capture.setToolTip(
            _("恢复为默认快捷键：") + str(Config.DEFAULTS["hotkey_capture"]))
        ...
        tip = QLabel(
            _("提示：快捷键请避免与其他软件冲突；划词取词通过模拟 Ctrl+C 读取"
              "选中文字，随后自动恢复剪贴板内容。"))
    ...
    def accept(self):
        ...
        self._config["language"] = self._cmb_lang.currentData()
        super().accept()
