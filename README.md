...
- **GitHub Releases（推荐）**：<https://github.com/godoge/TinyDict/releases>
- **夸克网盘**：<https://pan.quark.cn/s/9fc9c7c05e64>

下载后解压 `TinyDict-vX.X.X-win64.zip`，运行其中的 `TinyDict.exe` 即可。
（开源程序未做代码签名，Windows SmartScreen 可能拦截，点「仍要运行」或在文件属性中「解除锁定」即可，详见下方「手动打包」一节。）
...
安装依赖（推荐使用 **uv**，能避开 pip 在 Windows 上安装 PySide6 时偶发的"回收站安全删除"失败）：
...
└── tinydict/
    ...
    │   ├── settings_dialog.py     # 设置对话框（含语言切换）
    │   └── icons.py               # 程序图标（代码绘制）
    └── system/
